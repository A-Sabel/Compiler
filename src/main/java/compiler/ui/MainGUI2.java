...
public class MainGUI2 extends JFrame {
    ...
    private static final Color L_BORDER_COLOR    = new Color(0xD8DEE4);
    ...
    private static final Color L_OUTPUT_TAB_BG   = new Color(0x3C3C3C);
    ...
    private JPanel buildHeader() {
        ...
        left.add(logoBox); left.add(titlePanel);

        // ── Right: VSCode-style action buttons ────────────────────────────────
        JPanel right = new JPanel(new FlowLayout(FlowLayout.RIGHT, 4, 0));
        right.setOpaque(false);

        right.add(buildHeaderBtn("New File",     "\uE001", false, "New Project (Ctrl+N)"));   // new file icon-like
        right.add(buildHeaderBtn("Open",         "\uE178", false, "Load File (Ctrl+O)"));
        right.add(buildHeaderBtn("Run",          "\uE037", true,  "Run Compiler (F5)"));
    ...
    private JPanel buildEditorPanel() {
        ...
        fileTab.setPreferredSize(new Dimension(130, 36));
    ...
    private JButton buildEditorBtn(String icon, String label, String tooltip) {
        ...
        if (tooltip.contains("Clear")) {
            btn.addActionListener(e -> {
                ...
                String now = LocalTime.now().format(DateTimeFormatter.ofPattern("HH:mm:ss"));
                appendConsole("\uD83D\uDDD1 [" + now + "] Editor cleared", INFO_BLUE);
            });
        }
        return btn;
    }
    ...
    private JPanel buildOutputPanel() {
        ...
        outputHeader.setPreferredSize(new Dimension(0, 30));
        ...
        // Tab definitions — Token Table removed, AST Tree added
        String[][] tabs = {
            {"\u25A3", "Symbol Table"},
            {"\uD83C\uDF32", "AST Tree"},
            {"{ }", "Generated Code"},
        ...
        outputTabs.addTab(tabs[0][1], buildSymbolTablePanel());
        outputTabs.addTab(tabs[1][1], buildASTTreePanel());
        outputTabs.addTab(tabs[2][1], buildGeneratedCodePanel());
        outputTabs.addTab(tabs[3][1], buildRuntimeOutputPanel());
        outputTabs.addTab(tabs[4][1], buildOutputLogPanel());
        ...
        for (int i = 0; i < outputTabs.getTabCount(); i++) {
            ...
            iconLbl.setFont(new Font("Segoe UI Symbol", Font.PLAIN, 11));
            iconLbl.setForeground(i == 0 ? new Color(0xCCCCCC) : outTabFg());

            JLabel nameLbl = new JLabel(tabs[i][1]);
            nameLbl.setFont(FONT_UI_SM);
            nameLbl.setForeground(i == 0 ? new Color(0xCCCCCC) : outTabFg());
    ...
    private JPanel buildASTTreePanel() {
        ...
        JLabel title = new JLabel("\uD83C\uDF32  Abstract Syntax Tree");  // 🌲
        ...
        JPanel btnBar = new JPanel(new FlowLayout(FlowLayout.RIGHT, 4, 0));
        btnBar.setOpaque(false);
        btnBar.add(collapseBtn); btnBar.add(expandBtn);
        header.add(btnBar, BorderLayout.EAST);

        astArea = new JTextArea();
        astArea.setFont(FONT_MONO_SM);
        astArea.setBackground(bgPanel());
        astArea.setForeground(accentDark());
        astArea.setEditable(false);
        astArea.setBorder(BorderFactory.createEmptyBorder(8, 12, 8, 12));
        astArea.setText(
        ...
        JScrollPane scroll = new JScrollPane(astArea);
        scroll.setBorder(BorderFactory.createLineBorder(border()));
    ...
    // ── Output Log ────────────────────────────────────────────────────────────
    private JPanel buildOutputLogPanel() {
        JPanel p = new JPanel(new BorderLayout(0, 0));
        p.setBackground(bg());
        p.setBorder(BorderFactory.createEmptyBorder(8, 10, 8, 10));

        outputLogArea = new JTextArea();
        outputLogArea.setFont(FONT_MONO_SM);
        outputLogArea.setBackground(bgPanel());
        outputLogArea.setForeground(new Color(0xCCCCCC));
        outputLogArea.setEditable(false);
        outputLogArea.setBorder(BorderFactory.createEmptyBorder(8, 12, 8, 12));
        outputLogArea.setText("// Compilation log entries will appear here\n");

        JScrollPane scroll = new JScrollPane(outputLogArea);
        scroll.setBorder(BorderFactory.createLineBorder(border()));
        scroll.getViewport().setBackground(bgPanel());
        styleScrollBar(scroll.getVerticalScrollBar());
        p.add(scroll, BorderLayout.CENTER);
    ...
    private JPanel buildConsole() {
        ...
        strip.setPreferredSize(new Dimension(0, 32));
        strip.setOpaque(false);

        JPanel tabsLeft = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0));
        tabsLeft.setOpaque(false);
        tabsLeft.add(buildConsoleTabLbl(">_  Console", true));

        // Dynamic error / warning counts
        ...
        appendConsole("\u2139 [" + now + "] Ready for syntax analysis stage",         INFO_BLUE);

        JScrollPane scroll = new JScrollPane(consoleArea);
        scroll.setBorder(null);
        scroll.getViewport().setBackground(consoleBg());
        styleScrollBar(scroll.getVerticalScrollBar());
    ...
    private JPanel buildStatusBar() {
        ...
        lnColLabel.setFont(FONT_UI_SM); lnColLabel.setForeground(new Color(0xCCCCCC));

        JLabel enc  = new JLabel("UTF-8");
        enc.setFont(FONT_UI_SM); enc.setForeground(new Color(0xCCCCCC));

        JLabel lang = new JLabel("C");
